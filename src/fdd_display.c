...
void FDDdisplay_timer_init() {
  ...
  // TIM3 doesn't actually use an output pin, but I don't wanna risk it right now
  TIM3->CCMR1 &= ~(TIM_CCMR1_OC2CE | TIM_CCMR1_OC2M | TIM_CCMR1_OC2PE
        | TIM_CCMR1_OC2FE | TIM_CCMR1_CC2S);
...
void FDDdisplay_draw_full(uint8_t* prev, uint8_t* next) {
  // TODO finish draw function
  // process:
  // 1. ensure driver output enables all 0
  // 2. update shift regs
